...
- `GET /api/openapi.json` (control plane, senza autenticazione) — specifica **OpenAPI 3.0** generata dalla tabella di route (`routes.ts`): metodo, path e schema di sicurezza sono derivati meccanicamente (non possono disallinearsi dalle route realmente servite), riepiloghi e request body sono curati a mano. Utile per generare client o esplorare l'API con qualunque tool che legga OpenAPI (nessun renderer è servito dal control plane stesso, per non introdurre una dipendenza da CDN esterni).
...
Il rate limit dell'enrollment (`POST /api/enroll`) segue lo stesso backend: con Postgres il conteggio è condiviso tra tutte le istanze (`cp_rate_buckets`, upsert atomico), quindi la soglia (20/min per IP) resta quella dichiarata indipendentemente da quante istanze girano dietro il load balancer; in file-mode è in-memory (corretto solo a singola istanza). Tutti i rate limit usano una **finestra scorrevole** (weighted two-window): non lasciano passare il burst fino a 2× la soglia a cavallo di due finestre, come farebbe una finestra fissa.

Il rate limit del **gateway LLM** è condiviso tra le istanze del gateway se al gateway è passato `DATABASE_URL` (lo stesso Postgres del control plane, stessa `cp_rate_buckets`); senza, resta **per-istanza** (con N istanze la soglia effettiva è N×). Il compose imposta `DATABASE_URL` sul gateway per default. La condivisione usa la tabella Postgres direttamente (un upsert atomico sul percorso d'inferenza, nello stesso ordine di costo dell'introspezione già presente), non una chiamata sincrona al control plane.
...
- **Verificato dal vivo:** un token di abbonamento Anthropic (`claude setup-token`) presentato dal gateway riceve `401 "OAuth access token is invalid"` — è vincolato al client ufficiale e non è onorato da terzi su `/v1/messages`. Quindi la modalità OAuth **non** consente oggi di usare l'abbonamento Anthropic al posto dei crediti API: serve una API key. La modalità OAuth resta valida verso provider/gateway che accettano esplicitamente un bearer OAuth. OpenAI non ha alcun percorso OAuth per l'API.
...
Le sessioni sono **persistite tramite lo Store**: con backend Postgres (`cp_sessions`) sopravvivono a un restart e sono condivise tra le istanze (nessun login perso dietro un load balancer senza sticky session); in file-mode restano in memoria di processo (una sola istanza). Revocare il token amministrativo che ha aperto una sessione la **invalida subito** (`revokeAdminToken` → `deleteSessionsByToken`): non resta valida fino alla scadenza. `GET /api/admin/session/me` (sempre 200, mai un 401 rumoroso) permette alla UI di recuperare identità e CSRF token dopo un reload di pagina, finché il cookie resta valido; `POST /api/admin/session/logout` (che richiede il CSRF token, come ogni mutazione) invalida la sessione e cancella il cookie. Le sessioni scadute sono potate all'avvio, periodicamente e da `harness-cp prune`.
