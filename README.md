...
- **Audit tamper-evident**: ogni decisione di policy, redaction e comando utente è tracciata e centralizzata in log a catena di hash (manomissioni sempre rilevabili, `harness-cp verify-audit`); audit amministrativo separato per le modifiche di config. In caso di control plane irraggiungibile a lungo, il client bufferizza gli eventi e, oltre un tetto difensivo (per non esaurire la memoria), scarta i più vecchi: la copertura è quindi completa salvo un'indisponibilità prolungata del control plane.
